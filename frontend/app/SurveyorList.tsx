...
const SurveyorList = () => {
  ...
  const deleteSurveyor = async (id: string) => {
    try {
      await axios.delete(`http://192.168.0.183:8082/api/v1/surveyors/surveyors/${id}
`);
      fetchSurveyors(); // refresh list
    } catch (err) {
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          {surveyors.length === 0 ? (
            ...
            surveyors.map((s) => (
              <View key={s._id} className="bg-purple-50 border border-purple-200 p-4 rounded-xl mb-3">
                ...
                <TouchableOpacity onPress={() => deleteSurveyor(s._id)}>
                  <Text className="text-red-500 font-medium mt-2">🗑 Delete</Text>
                </TouchableOpacity>
              </View>
            ))
          )}
